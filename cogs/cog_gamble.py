    ...
    if store_to_processing(ctx):
        ...
        if success:
            ...
            if num <= 60:
                ...
                embed.set_author(name=f"{ctx.author.display_name} 抽到了 {num}, 輸了 {bet_amount} Nicoin， 剩餘 {balance} Nicoin", icon_url=ctx.author.display_avatar)
                if ctx.guild.icon:
                ...
                embed.set_author(name=f"{ctx.author.display_name} 抽到了 {num}, 贏了 {bet_amount} Nicoin. 現在有 {balance} Nicoin", icon_url=ctx.author.display_avatar)
            else:
                balance = db.get_balance(ctx.author.id, bet_amount*3)
                embed.colour = discord.Colour.gold()
                embed.set_author(name=f"{ctx.author.display_name} 抽到了 {num}, 贏了 {bet_amount*2} Nicoin. 現在有 {balance} Nicoin", icon_url=ctx.author.display_avatar)
