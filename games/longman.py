...
class Hit_Modal(Modal):
    def __init__(self, text: str, view: View, act: str, max_amount: int) -> None:
        super().__init__(text)
        self.add_item(InputText(label=f"Nicoins (max: {max_amount})", placeholder="Nicoins", value="", style=discord.InputTextStyle.short))
        self.view = view
        self.act = act
    ...
    for i, card in enumerate(player["cards"]):
        if player["revealed"] or force_show:
            if i < 2:
                ...
                if card != -1:
                    now_cards += f"\nshot: |:{deck_of_card[card]['suit']}:{deck_of_card[card]['number']}| "
                    cards_rnum.append(deck_of_card[card]['r_number'])
